...
private:
  ...
  matchFeatures(std::vector<cv::KeyPoint> &keypoints,
                const cv::Mat &descriptors,
                const std::vector<cv::KeyPoint> &previous_keypoints,
                const cv::Mat &previous_descriptors);
  std::vector<cv::DMatch>
  filterMatches(const std::vector<std::vector<cv::DMatch>> &knn_matches);
  int generateFeatureId();

  // Store new keypoints in feature tracks map, and delete expired tracks
  void storeFeatureTracks(const std::vector<cv::KeyPoint> &keypoints,
                          uint32_t image_seq);
